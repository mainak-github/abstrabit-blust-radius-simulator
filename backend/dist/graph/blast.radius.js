 ...
 */
function classifyImpact(depth, criticality) {
 ...
 */
function riskContribution(depth, criticality) {
    const depthWeight = Math.max(1, 6 - depth); // 1->5, 2->4, 3->3, ...
    return CRITICALITY_WEIGHT[criticality] * depthWeight * 5;
}
function severityFromScore(score, affected) {
    if (score >= 200 || affected >= 8)
        return "CRITICAL";
    if (score >= 100 || affected >= 5)
        return "HIGH";
    if (score >= 40 || affected >= 2)
        return "MEDIUM";
    return "LOW";
}
/**
 ...
 * Complexity: O(V + E)
 */
function computeBlastRadius(graph, failedIds) {
    ...
    while (queue.length > 0) {
        ...
        if (!node)
            continue;
        if (!impacted.has(id)) {
            const status = depth === 0 ? "FAILED" : classifyImpact(depth, node.criticality);
